...
- `{fieldname}` — アイテムの同名フィールドの値に置換されます（`{id}`・`{key}`・`{url}` など任意のフィールドが使えます）
- `{a|b}` — `a` フィールドが存在すれば `a` を、なければ `b` を使うフォールバック記法
- `|` は何段階でも連結できます（例: `{thumb_key|fallback|key}`）

> **`{id}` の活用例:** `thumbnail` テンプレートに `{id}` を使うと、アイテムの `id` をそのままシード値として利用できます。
> ```json
...
### id によるアイテム個別のフォーマット定義

フォーマットキーにアイテムの `id` を使うと、そのアイテム専用のURL・サムネイルを定義できます。`id` キーのエントリは `urlformat` フィールドより優先して適用されます。
...
この定義がある場合、`"id": "music-001"` のアイテムは `"urlformat"` の値に関わらずこのエントリが使われます。
